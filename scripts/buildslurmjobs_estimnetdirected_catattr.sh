#!/bin/sh
#
# File:    buildslurmjobs_estimnetdirected_catattr.sh
# Author:  Alex Stivala
...
#
# Usage: buildslurmjobs_estimnetdirected.sh  out_root_dir catattr_file in_networks_files_basename
#
...
#   config_catattr.template
#   estimnetdirected_slurm_script.template
#

SCRIPTDIR=`dirname $0`
PBS_TEMPLATE=${SCRIPTDIR}/estimnetdirected_slurm_script.template
SETTINGS_TEMPLATE=${SCRIPTDIR}/config_catattr.template

root=`dirname ${SCRIPTDIR}`

if [ $# -ne 3 ]; then
  echo "usage: $0 out_root_dir catattr_file in_networks_files_Basename" >&2
  exit 1
fi
out_root_dir=$1
catattr_file=$2
...
do
  ...
  catattrfilename=categoricalAttributes.txt
  cat ${catattr_file} > ${outdir}/${catattrfilename}
  settingfile=${outdir}/config.txt
  networkfilename=`basename ${networkfile}`
  settingfilename=`basename ${settingfile}`
  cat $SETTINGS_TEMPLATE | sed "s!@NETWORK!${networkfilename}!g" | sed "s!@CATEGORICALATTRIBUTES!${catattrfilename}!g"  > ${settingfile}
