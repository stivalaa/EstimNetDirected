#!/usr/bin/Rscript
##
## File:    plotEstimNetDirectedcERGMSimFit.R
## Author:  Alex Stivala
...
##
##
## Usage: Rscript plotEsimtNetDirectedcERGMSimFit.R [-s] netfilename termfilename simNetFilePrefix
...
##
## Output file is simfitPrefix_cergm.pdf (where Prefix is the simNetFilePrefix).
## WARNING: output file is overwritten
##
## Example:
## Rscript plotEsimtNetDirectedcERGMSimFit.R ../pythonDemo/polblogs/polblogs_arclist.txt sim_polblogs
...
if (length(args) > 4 || length(args) < 3) {
  cat("Usage: Rscript plotEstimNetDirectedcERGMSimFit.R [-s] netfilename termfilename simNetFilePrefix\n")
  quit(save="no")
...
outfilename <- paste(paste(simnetfileprefix, 'cergm', sep='_'), "pdf", sep='.')
