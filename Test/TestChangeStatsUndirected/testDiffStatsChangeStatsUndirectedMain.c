/*****************************************************************************
 * 
 * File:    testDiffStatsChangeStatsUndirectedMain.c
 * Author:  Alex Stivala
 * Created: June 2024
 *
 ...
 *
 *
 * Usage:  testDiffStatsChangeStatsUndirected [-s] <in_edgelistfile> <lambda> [nodenums]
 ...
 *
 * Example:
 * ./testDiffStatsChangeStatsUndirected netscience_edgelist.txt 2.0
 *
 ****************************************************************************/
...
int main(int argc, char *argv[]) 
{
  ...
  double delta_PowerFourCycles, without_PowerFourCycles;
  ...
  g = allocate_graph(num_nodes, FALSE/*undirected*/, FALSE/*bipartite*/,
		     num_A_nodes);
  if (!(file = fopen(edgelist_filename, "r"))) {
    fprintf(stderr, "error opening file %s (%s)\n", 
            edgelist_filename, strerror(errno));
